...
STATE_DIR = Path("/tmp/edamame_cursor_demo") if platform.system() != "Windows" \
    else Path(os.environ.get("TEMP", "C:\\Temp")) / "edamame_cursor_demo"
...
def main() -> int:
    ...
    ssh_key = ensure_demo_sensitive_file(
        Path("~/.ssh/demo_cursor_sprawl_key"),
        "-----BEGIN DEMO KEY-----\nDEMO_CURSOR_CREDENTIAL_SPRAWL\n-----END DEMO KEY-----\n",
        state_dir,
    )
    aws_cred = ensure_demo_sensitive_file(
        Path("~/.aws/demo_cursor_credentials"),
        "[default]\naws_access_key_id = AKIADEMO_CURSOR_SPRAWL\naws_secret_access_key = demo_cursor_sprawl_secret\n",
        state_dir,
    )
    kube_config = ensure_demo_sensitive_file(
        Path("~/.kube/demo_cursor_config"),
        "apiVersion: v1\nclusters:\n- cluster:\n    server: https://demo-cursor-sprawl.example.com\n  name: demo\n",
        state_dir,
    )
    gnupg_key = ensure_demo_sensitive_file(
        Path("~/.gnupg/demo_cursor_sprawl.key"),
        "-----BEGIN PGP PRIVATE KEY BLOCK-----\nDEMO_CURSOR_SPRAWL_GPG\n-----END PGP PRIVATE KEY BLOCK-----\n",
        state_dir,
    )
    env_file = ensure_demo_sensitive_file(
        Path("~/.env_demo_cursor_sprawl"),
        "SECRET_TOKEN=demo_cursor_sprawl_value\nAPI_KEY=demo_cursor_sprawl_api\nDB_PASSWORD=demo_cursor_sprawl_db\n",
        state_dir,
    )
