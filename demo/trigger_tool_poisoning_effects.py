...
def main() -> int:
    ...
    ssh_key = ensure_demo_sensitive_file(
        Path("~/.ssh/demo_cursor_tool_poison_key"),
        "DEMO_CURSOR_TOOL_POISONING_EXFIL_KEY\n",
        state_dir,
    )
    env_file = ensure_demo_sensitive_file(
        Path("~/.env_demo_cursor_tool_poison"),
        "API_TOKEN=demo_cursor_tool_poison_token\nSECRET=demo_cursor_tool_poison_secret\n",
        state_dir,
    )
